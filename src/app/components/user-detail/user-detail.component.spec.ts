...
import { of } from 'rxjs';
import IDisplayHeroDetail from '@/app/domain/ports/i-display-user-detail';
...
describe('HeroDetailComponent', () => {
    ...
    await TestBed.configureTestingModule({
      ...
      imports: [
        RouterTestingModule.withRoutes([
          { path: 'detail/0', component: HeroDetailComponent },
        ]),
      ],
    }).compileComponents();
  });
  ...
  it('should navigate back on go back button click', waitForAsync(() => {
    let button =
      fixture.debugElement.nativeElement.querySelector('#go-back-button');
    button.click();
  ...
  it('should ask hero name change navigate back on hero name change button click', waitForAsync(() => {
    ...
    fixture.whenStable().then(() => {
      expect(spyIDisplayHeroDetail.askHeroNameChange).toHaveBeenCalledOnceWith(
        'new_name'
      );
      expect(spyLocation.back).toHaveBeenCalledOnceWith();
    });
